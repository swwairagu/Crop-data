...
This dataset has `r nrow(lint_data)` rows and `r ncol(lint_data)` columns.

Each row represents a unique cotton plant, treatment observation. \
Each column represents a variable. To get a list of the variables in the data frame, we use the `names()` function:

```{r lint-var}
names(lint_data)
```

To analyze the lint quality data, we will follow the steps used in the analysis of the cotton data above. To streamline the analysis pipeline, we will need to write a function that takes a numeric vector as input and performs the steps of the analysis.


### Function 

```{r function}
analyze_data <- function(data, column_name){
 
  #1. Check distribution using a histogram
  hist_plot <- ggplot(data, aes(x = column_name)) +
    geom_histogram(bins = 13) +
    labs(title = "Histogram", x = "column_name", y = "Frequency")
  
  print(hist_plot)
  
  #2. Perform log transformation for skewed data
  log_data <- log(data(column_name))
  
  #3. Check for normality
  shapiro_test <- shapiro.test(log_data)
  print(shapiro_test)
  
  #4. Normal Quantile Plot
  qq_plot <- ggplot(log_data, aes(sample = log_data)) +
    stat_qq_point(size = 2) +
    stat_qq_line() +
    stat_qq_band(alpha = 0.5) +
  ...
  #5. Perform t-test or Wilcoxon Rank sum test based on Shapiro-Wilk test result
  if (shapiro_test$p-value > 0.05) {
    t_test <- t.test(log_data, var.equal=TRUE)   # Perform t-test
    print(t_test)
  }
  else {
    wilcox <- wilcox_test(data ~ Treatment)      # Perform Wilcoxon Rank Sum test
    print(wilcox)
  }
}
```
